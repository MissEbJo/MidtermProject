...
<body>
	...
	<form action="editGameInfo.do" method="GET">
		Name: <input type="text" name="name" id="name" value="${game.name }" required />
		<br> 
		<br> 
		...
		<input type="radio"
			name="singlePlayer" id="sptrue" value="true"
			checked="${game.singlePlayer }" /> 
			<label for="spfalse">No</label> 
			<input type="radio" name="singlePlayer" id="spfalse" value="false"
			checked="${game.singlePlayer }" /> 
			<br>
			Multiplayer - <label for="mptrue">Yes</label> 
			<input type="radio" name="multiPlayer" id="mptrue" value="true" checked="${game.multiPlayer }" /> 
			<label for="mpfalse">No</label> 
			<input type="radio" name="multiPlayer" id="mpfalse" value="false" checked="${game.multiPlayer }" /> <br>
		<br> Is this game cross-platform (can two players on two
		different systems play with each other)?: <br> <label for="crpl1">Yes</label>
		<input type="radio" name="crossPlatform" id="crpl1" value="true"
			checked="${game.crossPlatform }" /> <label for="crpl2">No</label> <input
			type="radio" name="crossPlatform" id="crpl2" value="false"
			checked="${game.crossPlatform }" /> <br> <br> Cover Art
		URL: <input type="URL" name="imageUrl" value="${game.imageUrl }" /> <br>
		<br> Game Trailer URL: <input type="URL" name="trailerUrl"
			value="${game.imageUrl }" /> <br> <br> What number in the
		series is this game? (Leave blank if the game is not in a series): <input
			type="number" name="numberInSeries" step="1"
			value="${game.numberInSeries }" /> <br> <br> What genre is
		this game: <br>
		...
		What Platform can you play your game on? <br>
		...
		<input type="reset"> <input type="submit" value="Add Game" />
